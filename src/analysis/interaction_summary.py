"""Summarize legacy ABG interaction experiment results."""
...
def summarize_interaction_results(infile, outdir):
    """Aggregate merged ABG simulation results using legacy metrics."""
    ...
    summary = (
        ...
        .agg(
            ...
            mean_time_to_explosion=(
                "time_to_explosion",
                lambda x: (
                    float(np.mean([v for v in x if v >= 0]))
                    if any(x >= 0)
                    else -1.0
                ),
            ),
    ...
    print(
        "Topologies:",
        sorted(summary["topology"].unique().tolist()),
    )
    print(
        "Alpha count:",
        summary["alpha_social"].nunique(),
    )
    print(
        "Beta count:",
        summary["beta"].nunique(),
    )
    print(
        "Gamma count:",
        summary["gamma"].nunique(),
    )
