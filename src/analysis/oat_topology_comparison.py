"""Compare topology outcomes in legacy OAT experiments."""
...
def compare_oat_topologies(
    infile="oat_summary/oat_summary_by_topology.csv",
    outdir="oat_topology_comparison",
):
    """Compute absolute and relative topology gaps for OAT outcomes."""
    ...
    for (param, value), sub in grouped:
        ...
        for metric in [
            ...
            if sub[metric].notna().any():
                ...
                row[f"{metric}_max_topology"] = sub.loc[
                    max_idx,
                    "topology",
                ]
                row[f"{metric}_min_topology"] = sub.loc[
                    min_idx,
                    "topology",
                ]
            else:
