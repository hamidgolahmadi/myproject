# -*- coding: utf-8 -*-
"""
Created on Fri Dec 19 14:39:55 2025
...
# -*- coding: utf-8 -*-
"""
PPO (Network-only) v3 for InfoNetworkBondEnv

Key improvements:
- Stronger critic using (mean, std, max) aggregation over agents
- Macro reward = mean risk over net_period micro-steps (scaled)
- Separate lrs + separate grad clipping
- Full 30-seed evaluation vs baselines

Requirements:
- numpy
- torch

Make sure InfoNetworkBondEnv is importable (e.g., from temp import InfoNetworkBondEnv).
"""
...
from temp import InfoNetworkBondEnv  # <-- change if your env module name differs
...
def critic_features(x_agents):
    """
    Build a richer global feature vector for the critic:
...
class NetCritic(nn.Module):
    """
    Centralized critic using rich aggregation (mean, std, max).
    """
    ...
    def forward(self, x_agents):
        xg = critic_features(x_agents)      # (3d,)
...
def sample_network_action(logits, w_logits, K):
    ...
    Returns:
      ...
      w:         (N,K) Tensor simplex weights (deterministic softmax)
      logp:      scalar log-prob for neighbors (sum over i,k)
    """
    N = logits.shape[0]
...
def macro_step(env, actor, critic, gamma_fin=5.0, reward_scale=1e6, device="cpu"):
    """
    One macro-step:
    - Choose network rewiring once (applied at the first micro-step)
    - Run env for net_period micro-steps with fixed financial policy

    Macro reward:
      R = - mean_{micro} risk_v(micro)  (scaled)
    """
    ...
    R_macro = -mean_risk * reward_scale
...
def ppo_update(actor, critic, optA, optC, rollout, device="cpu",
               ...
               clip_actor=1.0, clip_critic=0.5):
    """
