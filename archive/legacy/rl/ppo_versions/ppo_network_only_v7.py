# -*- coding: utf-8 -*-
"""
Created on Fri Dec 19 22:40:31 2025
...
---------------------
Key fix vs v6:
- Indegree capacity is enforced INSIDE sampling (no post-processing).
- Log-prob is computed for the EXACT executed neighbor choices by replaying the same sampling order.

Assumptions:
- You have InfoNetworkBondEnv in temp.py, compatible with your prior runs.
- Env rewires when (t % net_period == 0) and net_logits/net_w_logits are provided.

Run:
%runfile '.../ppo_network_only_v7.py' --wdir
"""
...
class NetActor(nn.Module):
    def __init__(self, d_in, N, K, hidden=128):
        ...
        self.head_w = nn.Linear(hidden, K)       # weight logits (kept for later)
...
def sample_neighbors_with_capacity(logits, K, indeg_cap, agent_order, generator=None):
    ...
    # Process agents in a fixed order to make logprob replayable
    for i in agent_order.tolist():
        ...
        for k in range(K):
            ...
            # forbid duplicates within the same row
            if len(chosen) > 0:
                mask[torch.tensor(chosen, dtype=torch.long, device=device)] = False

            # capacity constraint: can only choose nodes with remaining indegree > 0
            mask = mask & (indeg_remaining > 0)

            # fallback: if capacity is too tight, relax capacity but keep self/duplicate constraints
            ...
            # update global capacity if it was a capacity-feasible pick
...
def logprob_neighbors_with_capacity(logits, neighbors, K, indeg_cap, agent_order):
    """
    Compute log-prob of a fixed neighbor matrix under the current logits,
...
def macro_step(env, actor, critic, gamma_fin, rcfg, device="cpu", torch_gen=None):
    ...
    # Fixed agent order per macro-step (stored for correct logprob replay)
...
if __name__ == "__main__":
    ...
    print("=" * 110)
    print(
        f"CONFIG v7: horizon=1000 | gamma_fin={gamma_fin} | "
