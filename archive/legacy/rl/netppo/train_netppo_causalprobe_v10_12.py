# train_netppo_causalprobe_v10_12.py
# -*- coding: utf-8 -*-
...
def build_probe_c_rewards(
    gini_seq: np.ndarray,
    dP_seq: np.ndarray,
    riskS_clip_seq: np.ndarray,
    w_gini_probe: float,
    w_dP_probe: float,
    w_risk_probe: float,
):
    return -(
        float(w_gini_probe) * gini_seq
        + float(w_dP_probe) * dP_seq
        + float(w_risk_probe) * riskS_clip_seq
    )
...
def rollout_one(
    ...
    R_list, b_list = [], []
    dP_list, gini_list, riskS_clip_list = [], [], []
    ...
    for _ in range(cfg.rollout_len):
        ...
        riskS_clip_list.append(float(info["riskS_clip"]))
    ...
    r_probe = build_probe_c_rewards(
        gini_seq=np.asarray(gini_list, dtype=float),
        dP_seq=np.asarray(dP_list, dtype=float),
        riskS_clip_seq=np.asarray(riskS_clip_list, dtype=float),
        w_gini_probe=cfg.probe_w_gini,
        w_dP_probe=cfg.probe_w_dP,
        w_risk_probe=cfg.probe_w_risk,
    )
    ...
    batch = {
        ...
        "riskS_clip": torch.tensor(riskS_clip_list, dtype=torch.float32, device=device),
...
def train(cfg: Config):
    ...
    print("=" * 118)
    print("NetPPO-CausalProbe v10.12")
    ...
    print(f"Reward: PROBE_C | train_net={cfg.train_net} fix_gate={cfg.fix_gate}({cfg.fixed_gate_value})")
    print("Probe reward: - (w_gini * gini + w_dP * dP + w_risk * riskS_clip)")
    print(f"w_gini={cfg.probe_w_gini} w_dP={cfg.probe_w_dP} w_risk={cfg.probe_w_risk}")
    print("=" * 118)
    ...
    for it in range(cfg.n_iters):
        ...
        if it % 10 == 0:
            ...
            dP_mean_all = float(dP_all.mean()) if dP_all.size > 0 else 0.0
            ...
            r_probe_mean = float(batch["r_probe"].mean().item())
            gini_mean = float(batch["gini"].mean().item())
            riskS_clip_mean = float(batch["riskS_clip"].mean().item())

            print(
                f"iter={it:04d} | "
                f"r_reg={float(batch['r_reg'].mean().item()):+.4f}±{float(batch['r_reg'].std(unbiased=False).item()):.4f} | "
                f"r_probe_used={r_probe_mean:+.4f} "
                f"(gini={gini_mean:.4f}, dP={dP_mean_all:.4f}, riskS_clip={riskS_clip_mean:.4f}) | "
