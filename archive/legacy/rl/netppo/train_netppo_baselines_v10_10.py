...
# -*- coding: utf-8 -*-
"""
TRAIN Net-PPO baselines v10.9 (unified) — patched (clean)

Patches:
1) net_ent_coef default = 0.0
2) net_temperature default = 1.0
3) dP computed inside env as mean(|P_after - P_before|) on rewiring steps
4) rewiring starts at net_start_t (so t=0 is not rewired)

Usage:
  python train_netppo_baselines_v10_9.py --topology random_fixed --seed 0
  python train_netppo_baselines_v10_9.py --topology scale_free   --seed 0
  python train_netppo_baselines_v10_9.py --topology small_world  --beta 0.1 --seed 0
"""
...
def build_scale_free_network(N: int, K: int, rng: np.random.Generator) -> np.ndarray:
    indeg = np.ones(N, dtype=float)  # +1 smoothing
    P = np.zeros((N, N), dtype=float)
    for i in range(N):
...
class InfoNetworkBondEnvFinal:
    """
    dP computed as mean(|P_after - P_before|) on rewiring steps.
    Rewire happens when (t % net_period == 0) AND trainer passes neighbors/w.
    """
...
# =============================================================================
# Gate distribution (kept but fixed gate mode default)
# =============================================================================
...
def train(cfg: Config):
    ...
    print("=" * 118)
    print("TRAIN Net-PPO baselines v10.9 (unified) — PATCHED")
    ...
    for it in range(cfg.n_iters):
        ...
        if cfg.train_net and M > 0:
            ...
            for _ in range(cfg.net_mini_epochs):
                ...
                for start in range(0, M, cfg.net_minibatch):
                    ...
                    ent_mb = torch.stack(ent_mb_list).mean() if (cfg.net_ent_coef > 0 and ent_mb_list) else torch.zeros((), device=device)
        ...
        if it % 10 == 0:
            ...
            print(
                ...
                f"dbg_last[dP]={dbg['dP']:.6f} dbg[gini]={dbg['gini']:.3f} dbg[riskS]={dbg['riskS']:.3g}"
            )
...
def parse_args():
    ...
    p.add_argument("--beta", type=float, default=0.1, help="small_world beta")
    p.add_argument("--seed", type=int, default=0, help="base seed (used for topo/env/train unless overridden)")
