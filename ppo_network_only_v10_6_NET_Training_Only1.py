# -*- coding: utf-8 -*-
"""
Created on Sat Jan 10 23:57:48 2026
...
# -*- coding: utf-8 -*-
"""
ppo_network_only_v10_8.py

v10.8 — APPLY ACTIONS (1)(2)(3)

(1) BASELINE: ability to run FIX_NET=True rollout (no rewiring) and print r_net stats alongside training run.
(2) REWARD FIX: default rewiring cost smaller (lambda_dP=0.1 and/or gamma_rew=0.1) so info/pol signal isn't choked.
(3) BETTER NET LOGS:
    - dP mean over rollout (already exists via env info; now also averaged)
    - probs_max (top-1 neighbor prob) mean/std
    - neighbor entropy mean/std
    - logits mean/std

Story:
- GateActor = Regulator with LIMITED obs (public-ish)
- NetActor  = Platform/Market optimizing information quality with RICH obs

Modes:
- TRAIN_GATE / TRAIN_NET
- FIX_GATE (constant g) / FIX_NET (no rewiring)
"""
...
# =============================================================================
# Environment (two observations; regulator reward computed inside)
# =============================================================================

class InfoNetworkBondEnvV10_8:
    """
    Mechanics:
    ...
    IMPORTANT v10.8 FIX:
      - P_prev is updated EACH STEP to represent previous-step P.
        => if FIX_NET (no rewiring), then dP ~ 0 each step (correct).
    """
    ...
    def step(self, g_gate, neighbors=None, w=None):
        # v10.8: P_prev is always previous-step P
        ...
        self.P_prev = P_old_step  # now P_prev is truly previous-step P
...
# =============================================================================
# Squashed Gaussian gate (kept for when TRAIN_GATE=True)
# =============================================================================
...
# =============================================================================
# Rollout (supports FIX_GATE / FIX_NET + debug stats)
# =============================================================================

def rollout_one(env: InfoNetworkBondEnvV10_8,
                gate_actor: GateActor,
                net_actor: NetActor,
                step_critic: StepCritic,
                cfg,
                force_fix_net: bool = False):
    device = cfg.device
    ...
    dbg_last = {}
    net_dbg_last = None  # store stats for last net step in rollout

    # action (3): also collect dP mean across rollout
    dP_roll = []
    ...
    for _ in range(cfg.rollout_len):
        ...
        # ---------------- Gate action ----------------
        if cfg.FIX_GATE:
            gate = torch.full((env.N,), float(cfg.FIXED_G), device=device)
        ...
        # ---------------- Net action ----------------
        fix_net_now = bool(cfg.FIX_NET or force_fix_net)

        do_net = ((env.t % cfg.net_period) == 0) and (not fix_net_now)
    ...
    batch = {
        ...
        # action (3): rollout-level dP summary
        "dP_roll_mean": float(np.mean(dP_roll)) if len(dP_roll) else 0.0,
        "dP_roll_std": float(np.std(dP_roll)) if len(dP_roll) else 0.0,
    }
    return batch


# =============================================================================
# Macro aggregation for Net-PPO (aggregates r_net)
# =============================================================================
...
class ConfigV10_8:
    ...
    # ACTION (2): rewiring cost smaller by default
    gamma_rew: float = 0.1       # was 1.0
    lambda_dP: float = 0.1       # was 1.0
    ...
    FIX_NET: bool = False   # if True: never rewire (net_actor unused in env)

    # ACTION (1): baseline rollouts (no rewiring) printed every PRINT_EVERY iters
    RUN_BASELINE_FIX_NET: bool = True
...
def train_v10_8():
    ...
    print("=" * 118)
    print(f"CONFIG v10.8: N={cfg.N} horizon={cfg.horizon} | gamma_fin={cfg.gamma_fin} | K={cfg.K} indeg_cap={cfg.indeg_cap} net_period={cfg.net_period}")
    print("Obs split: obs_reg (LIMITED) for GateActor | obs_net (RICH) for NetActor")
    print("Platform reward: alpha*info + beta*(-Var(b)) + gamma*(-lambda_dP*dP)")
    print(f"alpha={cfg.alpha_info} beta={cfg.beta_pol} gamma={cfg.gamma_rew} lambda_dP={cfg.lambda_dP} H={cfg.info_horizon}")
    print(f"MODE: TRAIN_GATE={cfg.TRAIN_GATE} TRAIN_NET={cfg.TRAIN_NET} | FIX_GATE={cfg.FIX_GATE}({cfg.FIXED_G}) FIX_NET={cfg.FIX_NET}")
    print(f"BASELINE: RUN_BASELINE_FIX_NET={cfg.RUN_BASELINE_FIX_NET} (printed every {cfg.PRINT_EVERY})")
    print("=" * 118)

    seed_train = 0
    set_seed(seed_train)

    env = InfoNetworkBondEnvV10_8(seed=seed_train, **env_kwargs)
    ...
    for it in range(cfg.n_iters):
        # Main rollout uses cfg.FIX_NET (training scenario)
        batch = rollout_one(env, gate_actor, net_actor, step_critic, cfg, force_fix_net=False)
        ...
        # ============================
        # (B) Net update (optional)
        # ============================
        # freeze diagnostics snapshot (BEFORE net update)
        with torch.no_grad():
        ...
        if cfg.TRAIN_NET and (not cfg.FIX_NET):
            r_net = batch["r_net"]
        ...
        # ============================
        # ACTION (1): baseline rollout (FIX_NET=True) for comparison
        # ============================
        baseline_stats = None
        if cfg.RUN_BASELINE_FIX_NET and (it % cfg.PRINT_EVERY == 0):
            # force_fix_net=True => no rewiring regardless of cfg.FIX_NET
            base = rollout_one(env, gate_actor, net_actor, step_critic, cfg, force_fix_net=True)
            baseline_stats = dict(
                r_net=float(base["r_net"].mean().item()),
                r_info=float(base["r_info"].mean().item()),
                r_pol=float(base["r_pol"].mean().item()),
                r_rew=float(base["r_rew"].mean().item()),
                dP_mean=float(base["dP_roll_mean"]),
                dP_std=float(base["dP_roll_std"]),
            )
        ...
        if it % cfg.PRINT_EVERY == 0:
            ...
            # gate reporting (even if fixed)
            gate_mean = float(batch["gate"].mean().item())
            gate_std = float(batch["gate"].std(unbiased=False).item())

            net_ratio_m = float(np.mean(net_ratio_means)) if net_ratio_means else 0.0
            ...
            # action (3): dP mean/std across rollout
            dP_mean = float(batch["dP_roll_mean"])
            dP_std = float(batch["dP_roll_std"])

            base_txt = ""
            if baseline_stats is not None:
                base_txt = (
                    f" | BASE(FIX_NET): r_net={baseline_stats['r_net']:+.4f} "
                    f"(info={baseline_stats['r_info']:+.4f}, pol={baseline_stats['r_pol']:+.4f}, rew={baseline_stats['r_rew']:+.4f}) "
                    f"dP={baseline_stats['dP_mean']:.4f}±{baseline_stats['dP_std']:.4f}"
                )
            ...
            print(
                ...
                f"dP_roll={dP_mean:.4f}±{dP_std:.4f} | "
                f"NET(TRAIN={cfg.TRAIN_NET}, FIX={cfg.FIX_NET}, M={M}): "
                ...
                f"GATE(TRAIN={cfg.TRAIN_GATE}, FIX={cfg.FIX_GATE}): gate={gate_mean:.3f}±{gate_std:.3f} | "
