from dataclasses import replace

import pytest

from src.experiments.refined.beta_sweep_analysis import (
    BetaSweepTreatmentRecord,
    analyse_beta_sweep_records,
)
from src.experiments.refined.beta_sweep_protocol import BetaSweepProtocol
from src.experiments.refined.confirmatory_runner import ConfirmatoryTreatmentRecord


def _protocol():
    return BetaSweepProtocol(
        experiment_seed=95001,
        alpha_anchor=0.85,
        beta_grid=(0.0, 1.0, 2.0, 5.0, 1000.0),
        n_replications=2,
        bootstrap_seed=95002,
        n_bootstrap=1000,
    )


def _record(seed: int, replication_id: int, beta: float, topology: str):
    offsets = {"R": 1.0, "SW": 0.0, "SF": 2.0}
    level = 1.0 + 0.01 * beta + offsets[topology] + 0.1 * replication_id
    signed = {"R": 0.1, "SW": -0.1, "SF": 0.2}[topology] + 0.001 * beta
    graph_offset = {"R": 1, "SW": 2, "SF": 3}[topology]
    treatment = ConfirmatoryTreatmentRecord(
        experiment_seed=seed,
        replication_id=replication_id,
        regime="beta_sweep",
        alpha=0.85,
        topology_label=topology,
        graph_seed=1000 + 10 * replication_id + graph_offset,
        shock_seed=2000 + replication_id,
        initial_state_seed=3000 + replication_id,
        economic_path_fingerprint=f"{beta}-{topology}-{replication_id}".ljust(64, "0"),
        return_volatility=level,
        rms_mispricing=level,
        maximum_absolute_mispricing=level,
        mean_absolute_order_flow_per_agent=level,
        mean_absolute_return=level,
        time_averaged_belief_variance=level,
        peak_cid=level,
        threshold_exceeding=(beta >= 5.0 and topology == "SF"),
        cid_exceedance_duration_share=0.01 * min(beta, 10.0),
        stabilised=True,
        stabilisation_period=3,
        right_censored=False,
        mean_pairwise_action_covariance=signed,
        mean_sum_individual_action_variances=level,
        mean_aggregate_order_flow_variance=10.0 * level,
        in_degree_gini=0.1,
        hub_link_share=0.1,
        global_clustering=0.1,
        average_path_length_lcc=2.0,
        largest_component_share=1.0,
        mean_attention_entropy=0.5,
        mean_effective_sources=2.0,
        mean_influence_hhi=0.1,
        mean_hub_influence_share=0.2 * level,
        mean_attention_overlap=0.05 * level,
        mean_attention_mobility=0.01,
    )
    return BetaSweepTreatmentRecord(beta=beta, treatment=treatment)


def _records(protocol=None):
    protocol = protocol or _protocol()
    return tuple(
        _record(protocol.experiment_seed, replication_id, beta, topology)
        for replication_id in range(protocol.n_replications)
        for beta in protocol.beta_grid
        for topology in protocol.topology_labels
    )


def test_beta_sweep_means_pairwise_signs_and_crn_flag_are_correct():
    result = analyse_beta_sweep_records(_records(), protocol=_protocol())
    assert result.cross_beta_common_random_numbers_verified is True
    means = {
        (item.beta, item.outcome, item.topology): item.estimate
        for item in result.topology_means
    }
    assert means[(5.0, "return_volatility", "SW")] == pytest.approx(1.10)
    assert means[(5.0, "return_volatility", "R")] == pytest.approx(2.10)
    assert means[(5.0, "return_volatility", "SF")] == pytest.approx(3.10)

    contrasts = {
        (item.beta, item.outcome, item.topology_left, item.topology_right): item.estimate
        for item in result.pairwise_contrasts
    }
    assert contrasts[(5.0, "return_volatility", "R", "SW")] == pytest.approx(1.0)
    assert contrasts[(5.0, "return_volatility", "R", "SF")] == pytest.approx(-1.0)
    assert contrasts[(5.0, "return_volatility", "SW", "SF")] == pytest.approx(-2.0)


def test_beta_sweep_bootstrap_is_reproducible():
    first = analyse_beta_sweep_records(_records(), protocol=_protocol())
    second = analyse_beta_sweep_records(_records(), protocol=_protocol())
    assert first.topology_means[10].ci_lower == second.topology_means[10].ci_lower
    assert first.topology_means[10].ci_upper == second.topology_means[10].ci_upper
    assert first.pairwise_contrasts[20].ci_lower == second.pairwise_contrasts[20].ci_lower


def test_binary_and_signed_outcomes_do_not_get_relative_effects():
    result = analyse_beta_sweep_records(_records(), protocol=_protocol())
    binary = next(
        item for item in result.pairwise_contrasts
        if item.beta == 5.0 and item.outcome == "threshold_exceeding"
    )
    signed = next(
        item for item in result.pairwise_contrasts
        if item.beta == 5.0 and item.outcome == "mean_pairwise_action_covariance"
    )
    assert binary.relative_effect is None
    assert signed.relative_effect is None


def test_incomplete_beta_block_is_rejected():
    records = list(_records())
    records.pop()
    with pytest.raises(ValueError, match="complete R/SW/SF triplet"):
        analyse_beta_sweep_records(records, protocol=_protocol())


def test_wrong_experiment_seed_is_rejected():
    records = list(_records())
    records[0] = BetaSweepTreatmentRecord(
        beta=records[0].beta,
        treatment=replace(records[0].treatment, experiment_seed=999),
    )
    with pytest.raises(ValueError, match="experiment seed"):
        analyse_beta_sweep_records(records, protocol=_protocol())


def test_cross_beta_seed_mismatch_is_rejected():
    records = list(_records())
    target = next(
        i for i, record in enumerate(records)
        if record.treatment.replication_id == 0
        and record.beta == 5.0
        and record.treatment.topology_label == "SF"
    )
    records[target] = BetaSweepTreatmentRecord(
        beta=records[target].beta,
        treatment=replace(records[target].treatment, shock_seed=99999),
    )
    with pytest.raises(ValueError, match="cross-beta shock-seed mismatch"):
        analyse_beta_sweep_records(records, protocol=_protocol())


def test_wrong_alpha_anchor_is_rejected():
    records = list(_records())
    records[0] = BetaSweepTreatmentRecord(
        beta=records[0].beta,
        treatment=replace(records[0].treatment, alpha=0.75),
    )
    with pytest.raises(ValueError, match="alpha"):
        analyse_beta_sweep_records(records, protocol=_protocol())
