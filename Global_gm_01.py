# -*- coding: utf-8 -*-
"""
Paper1 | Global g_t + Band-triggered q_t + CB inventory(cap-safe) + Fee Fund | PPO
--------------------------------------------------------------------------------
What’s in THIS version (your last requests):
1) CB inventory update is CAP-SAFE:
      q_applied = clip(q_raw, -Xcap - Xcb, +Xcap - Xcb)
      Xcb <- Xcb + q_applied
   (so q is reduced BEFORE applying; no “fake q then clip inventory”.)

2) Followers have LIGHT friction:
   - Transaction fee paid each step:
        fee_i,t = tau_fee * p_t * |a_i,t|
     Total fees go to a fund:
        Fund <- Fund + sum_i fee_i,t
   - Holding/financing cost is represented (lightweight) by inventory pullback:
        mu_eff = g*mu - eta_inv * x
     (eta_inv is your inventory carrying pressure; prevents infinite leverage / drift)
   - And we add an endogenous fee threshold into BR via tau_eff = tau0 + tau_fee*p

3) Global g_t (scalar) learned by PPO.
4) q_t is deterministic band-triggered intervention, global, and penalized in MM reward.
5) Outcome-only MM state: [|d|, var_ewma, mean|a|, I_band, |Xcb|] (all scalars)

Run with:
%runfile '.../Global_gm_02_fee_fund.py' --wdir
"""
...
class Config:
    ...
    # MM reward weights (normalized by running scales -> here keep simple weights + RMS scaling below)
    w_dist: float = 1.0
    w_var: float = 0.50
    w_imp: float = 1.0
    w_liq: float = 1.0
...
class GlobalGateBandCBEnv:
    """
    MM chooses scalar g_t in [0,1].
    Followers compute BR a_i*(g_t, state) with:
      mu_eff = g_t * mu_i - eta_inv * x_i
      mu_i = mu_scale * (alpha_fund * p) * m_i
      tau_eff = tau0 + tau_fee * p   (fee pushes threshold up)

      a_i = clip( Soft(mu_eff, tau_eff) / (2*(lam0 + lam1*g_t)), [-a_max, a_max] )

    CB intervention (deterministic, band-triggered, cap-safe):
      d_t = (p - p_fund) / p_fund
      I_t = 1{|d_t| > delta}
      q_raw = - I_t * k_q * clip(d_t, [-d_clip, d_clip])
      q_raw clipped to [-q_max, q_max]
      q_applied cap-safe w.r.t Xcb_cap

    Price:
      p_{t+1} = p_t + fund_move + kappa*(sum(a)+q_applied) + eps
      p_fund updates without impact and eps.

    Fund:
      fee_total_t = tau_fee * p_t * sum_i |a_i|
      Fund <- Fund + fee_total_t
    """
    ...
    def followers_best_response(self, g: float) -> np.ndarray:
        """
        BR with fee embedded via tau_eff = tau0 + tau_fee*p (L1 trading cost).
        """
        p = float(self.p)
        mu = self.cfg.mu_scale * (self.cfg.alpha_fund * p) * self.m  # signal incentive
        mu_eff = (g * mu) - self.cfg.eta_inv * self.x               # holding/financing cost proxy

        tau_eff = self.cfg.tau0 + self.cfg.tau_fee * p              # <-- fee enters decision
    ...
    def _compute_q_cap_safe(self, d_prev: float) -> Dict[str, float]:
        """
        Deterministic q law with band trigger and CAP-SAFE application.
        """
        cfg = self.cfg
        ...
        # CAP-SAFE: shrink q so Xcb won't cross cap
    ...
    def step(self, g_action: float, debug: bool = False):
        ...
        # distortion at START of step (use prev state to decide intervention)
        ...
        # Transaction fee -> Fund (your formula)
        fee_total = float(cfg.tau_fee * p_prev * np.sum(np.abs(a)))
        self.fund += fee_total

        # CB intervention (cap-safe)
        q_pack = self._compute_q_cap_safe(d_prev)
        q = q_pack["q"]
        self.Xcb = float(self.Xcb + q)  # safe by construction
        ...
        # Fundamental shadow price (no impact, no eps)
        p_f_new = p_f_prev + fund_move

        # Update follower inventory (with cap)
        ...
        # MM loss/reward (note: no arbitrary scaling here; you can add later if you want)
        mm_loss = (
            cfg.w_dist * dist2
        ...
        if debug:
            print(
                f"[t={self.t:04d}] g={g:.3f} | d_prev={d_prev:+.4%} I={int(q_pack['I'])} "
                f"q={q:+.3f} Xcb={self.Xcb:+.2f} | sum_a={sum_a:+.3f} mean|a|={mean_abs_a:.3f} "
                f"feeTot={fee_total:.3f} fund={self.fund:.2f} | impact={impact:+.4f} "
                f"p={p_new:.3f} p_f={p_f_new:.3f} d_new={d_new:+.4%} | loss={mm_loss:.4f} rew={reward:.4f}"
            )
    ...
    def _get_obs(self):
        """
        Outcome-only MM state (all scalars):
          s_t = [|d|, var_ewma, mean|a|_prev, I_band, |Xcb|]
        """
        cfg = self.cfg
...
class Actor(nn.Module):
    ...
    def logp_entropy(self, x, act):
        d = self.dist(x)
        return d.log_prob(act), d.entropy()
...
class PPOAgent:
    ...
    def update(self):
        ...
        obs = np.asarray(self.obs, dtype=np.float32)          # (T,D)
        act = np.asarray(self.act, dtype=np.float32)          # (T,)
        logp_old = np.asarray(self.logp, dtype=np.float32)    # (T,)
        rew = np.asarray(self.rew, dtype=np.float32)          # (T,)
        done = np.asarray(self.done, dtype=np.float32)        # (T,)
        next_obs = np.asarray(self.next_obs, dtype=np.float32)# (T,D)
...
def train_ppo(cfg: Config):
    ...
    print("\n=== Train PPO MM (global g + band q + cap-safe Xcb + fee fund) ===")
        ...
        for t in range(cfg.T):
            ...
            if done:
                ...
                if ep % 10 == 0:
                    R = np.asarray(env.price_ret_hist, dtype=np.float64)
                    print(
                        f"ep={ep:04d} | ep_rew={ep_rew:+.2f} | g_mean={np.mean(env.g_hist):.3f} | "
...
def main():
    ...
    print("=" * 110)
    print("Paper1 | Global g_t + Band q_t + CAP-SAFE CB inventory + Transaction Fee Fund | PPO")
    print("=" * 110)
    ...
    for g in grid:
        ...
        if best is None or r["ep_rew"] > best["ep_rew"]:
            best = r
        print(
            f"{r['g']:.1f} | {r['ep_rew']:+8.1f} | {r['Sharpe']:+6.3f} | {r['d_rms']:7.3%} | "
            f"{r['mean_abs_a']:7.3f} | {r['mean_sum_a']:+11.3f} | {r['mean_q']:+7.3f} | "
            f"{r['Xcb_end']:+7.2f} | {r['Fund_end']:8.2f}"
        )

    print(f"\nBest fixed g by ep_rew: g={best['g']:.2f} ep_rew={best['ep_rew']:+.2f}")
    ...
    print(
        f"ep_rew={out['ep_rew']:+.2f} | g_mean={out['g_mean']:.3f} | Sharpe={out['Sharpe']:+.3f} | "
